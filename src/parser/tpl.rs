...
pub fn tpl_set_ref_code(r: Box<Expr>, host_ident: Ident) -> Box<Expr> {
  let args = vec![
    ast_create_arg_expr(ast_create_expr_ident(host_ident)),
...
pub fn tpl_push_el_code(root: bool, host_ident: Ident) -> Box<Expr> {
  ...
  Box::new(Expr::Call(CallExpr {
    ...
    callee: Callee::Expr(ast_create_expr_member(
      ast_create_expr_member(
        ast_create_expr_ident(host_ident),
...
pub fn tpl_render_expr_text(
  ...
  let mut stmts = vec![
    ...
    Stmt::Expr(ExprStmt {
      span: DUMMY_SP,
      expr: tpl_watch_and_render(render_fn, expr_result, host_ident.clone()),
    }),
  ];
...
pub fn tpl_watch_and_render(
  ...
  match expr_result {
    ...
    ExprParseResult::Simple(sr) => {
      ...
      args.push(ast_create_arg_expr(ast_create_expr_ident(host_ident)));
      ast_create_expr_call(
