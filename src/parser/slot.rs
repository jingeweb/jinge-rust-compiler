...
fn get_slot_name_type_from_optchain_expr(
  expr: &OptChainExpr,
  props_arg: &Option<Atom>,
) -> SlotNameType {
  match expr.base.as_ref() {
    OptChainBase::Member(m) => get_slot_name_type_from_member_epxr(m, props_arg),
    OptChainBase::Call(c) => match c.callee.as_ref() {
      Expr::Member(mem) => get_slot_name_type_from_member_epxr(mem, props_arg),
      Expr::OptChain(opt) => get_slot_name_type_from_optchain_expr(opt, props_arg),
      _ => SlotNameType::None,
    },
  }
}
...
impl TemplateParser {
  ...
  pub fn parse_slot_optchain_expr(&mut self, expr: &OptChainExpr) -> bool {
    match get_slot_name_type_from_optchain_expr(expr, &self.props_arg) {
      SlotNameType::None => false,
      SlotNameType::Default => {
        let slot_name = Box::new(Expr::OptChain(OptChainExpr {
          span: DUMMY_SP,
          optional: true,
          base: Box::new(OptChainBase::Member(get_default_slot_name(
            self.props_arg.as_ref().unwrap(),
          ))),
        }));
      ...
      SlotNameType::Expr => {
        self.transform_slot(Box::new(Expr::OptChain(expr.clone())), None);
        true
      }
    }
  }

  fn get_bin_expr_slot_name(&self, expr: &Expr) -> Option<Box<Expr>> {
  ...
  pub fn parse_nullish_coalescing_slot(&mut self, expr: &BinExpr) -> bool {
    let slot_name;
    let slot_args;
    if let Some(sn) = self.get_bin_expr_slot_name(&expr.left) {
      slot_name = sn;
      slot_args = None;
    } else {
      ...
      match expr.left.as_ref() {
        Expr::Call(call) => match &call.callee {
          Callee::Expr(e) => {
            if let Some(sn) = get_slot_name_from_callee(e, &self.props_arg) {
              slot_name = sn.clone();
              slot_args = Some(&call.args);
            } else {
              return false;
            }
          }
        ...
        Expr::OptChain(opt) => match opt.base.as_ref() {
          OptChainBase::Call(c) => {
            if let Some(sn) = get_slot_name_from_callee(&c.callee, &self.props_arg) {
              slot_name = sn.clone();
              slot_args = Some(&c.args);
            } else {
              return false;
            }
          }
